...
import type { PanoramaCaptureOptions } from "../utils/canvas-panorama-capture";
import type { PanoramaViewState } from "../components/canvas-panorama-viewer";
...
function copyCanvasNodeMetadata(metadata?: CanvasNodeMetadata): CanvasNodeMetadata | undefined {
    if (!metadata) return undefined;
    const hasRunningTask =
        metadata.status === NODE_STATUS_LOADING ||
        Boolean(metadata.imageCandidateBatches?.some((batch) => batch.items.some((candidate) => candidate.status === NODE_STATUS_LOADING))) ||
        Boolean(metadata.videoCandidateBatches?.some((batch) => batch.items.some((candidate) => candidate.status === NODE_STATUS_LOADING)));
    if (!hasRunningTask) return { ...metadata };

    const imageCandidateBatches = metadata.imageCandidateBatches
        ?.map((batch) => ({
            ...batch,
            items: batch.items
                .filter((candidate) => candidate.status !== NODE_STATUS_LOADING)
                .map(({ imageTaskId, imageTaskResultId, startedAt, progress, ...candidate }) => candidate),
        }))
        .filter((batch) => batch.items.length);
    const videoCandidateBatches = metadata.videoCandidateBatches
        ?.map((batch) => ({
            ...batch,
            items: batch.items
                .filter((candidate) => candidate.status !== NODE_STATUS_LOADING)
                .map(({ videoTaskId, videoTaskVideoId, startedAt, progress, ...candidate }) => candidate),
        }))
        .filter((batch) => batch.items.length);
    const { imageTaskId, imageTaskResultId, audioTaskId, audioTaskResultId, videoTaskId, videoTaskVideoId, startedAt, durationMs, progress, ...next } = metadata;
    return {
        ...next,
        status: metadata.status === NODE_STATUS_LOADING ? (metadata.content ? NODE_STATUS_SUCCESS : "idle") : metadata.status,
...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const duplicateNode = useCallback((nodeId: string) => {
        ...
        const next: CanvasNodeData = {
            ...
            metadata: copyCanvasNodeMetadata(source.metadata),
        };
    ...
    const copySelectedNodes = useCallback(() => {
        ...
        const copiedNodes = nodesRef.current
            ...
            .map((node) => ({
                ...
                metadata: copyCanvasNodeMetadata(node.metadata),
            }));
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            const prompt = `参考图中彩色标记的局部区域是需要修改的位置；实色涂抹仅修改高亮部分，矩形边框则修改边框所圈范围。标记颜色只是编辑提示，不要保留在最终图像中。其他区域的构图、人物、文字、光影和风格保持不变。修改要求：${userPrompt}`;
            const childId = nanoid();
                            ...
                            renderPanel={(panelNode) =>
                                panelNode.type === CanvasNodeType.Config ? (
                                    <CanvasConfigComposer
                                        ...
                                        readOnly={isCanvasNodeGenerating(panelNode) || runningNodeId === panelNode.id}
...
function FullscreenPreview({ src, alt, captureId, onCapturePanorama, isPanorama, proxyGeneratedPanorama = false, onClose, hasPrev, hasNext, onPrev, onNext }: { src: string; alt: string; captureId?: string; onCapturePanorama?: (dataUrl: string) => void; isPanorama?: boolean; proxyGeneratedPanorama?: boolean; onClose: () => void; hasPrev?: boolean; hasNext?: boolean; onPrev?: () => void; onNext?: () => void }) {
    ...
    if (isPanorama) {
        return <PanoramaFullscreenPreview src={src} alt={alt} captureId={captureId} proxyGeneratedPanorama={proxyGeneratedPanorama} onCapturePanorama={onCapturePanorama} onClose={onClose} hasPrev={hasPrev} hasNext={hasNext} onPrev={onPrev} onNext={onNext} />;
    }
...
type PanoramaCaptureResolution = "1K" | "2K" | "4K";
type PanoramaCaptureAspect = "16:9" | "1:1" | "4:3" | "3:4" | "9:16" | "21:9";
type PanoramaPreviewMode = "simple" | "professional";
type PanoramaCameraState = { x: number; y: number; z: number; yaw: number; pitch: number; roll: number; fov: number };

const PANORAMA_RESOLUTION_BASE: Record<PanoramaCaptureResolution, number> = { "1K": 1024, "2K": 2048, "4K": 4096 };
...
function panoramaCaptureSize(resolution: PanoramaCaptureResolution, aspect: PanoramaCaptureAspect): PanoramaCaptureOptions {
    ...
    const ratio = PANORAMA_ASPECT_RATIO[aspect];
    return ratio >= 1 ? { width: base, height: Math.max(1, Math.round(base / ratio)) } : { width: Math.max(1, Math.round(base * ratio)), height: base };
}

function PanoramaFullscreenPreview({ src, alt, captureId, proxyGeneratedPanorama, onCapturePanorama, onClose, hasPrev, hasNext, onPrev, onNext }: { src: string; alt: string; captureId?: string; proxyGeneratedPanorama: boolean; onCapturePanorama?: (dataUrl: string) => void; onClose: () => void; hasPrev?: boolean; hasNext?: boolean; onPrev?: () => void; onNext?: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    ...
    const capture = () => {
        const dataUrl = captureId ? capturePanorama(captureId, captureSize) : null;
        if (dataUrl) onCapturePanorama?.(dataUrl);
    };

    const fieldStyle = { background: theme.node.fill, borderColor: theme.toolbar.border, color: theme.node.text };
    const selectClass = "h-8 min-w-20 rounded-md border px-2 text-xs outline-none transition focus:border-white/50";
    const cameraFields: Array<{ key: keyof PanoramaCameraState; label: string; suffix: string; min: number; max: number; step: number }> = [
        { key: "x", label: "X", suffix: "", min: -100, max: 100, step: 0.1 },
        { key: "y", label: "Y", suffix: "", min: -100, max: 100, step: 0.1 },
        { key: "z", label: "Z", suffix: "", min: -100, max: 100, step: 0.1 },
        { key: "yaw", label: "Yaw", suffix: "°", min: -180, max: 180, step: 1 },
        { key: "pitch", label: "Pitch", suffix: "°", min: -90, max: 90, step: 1 },
        { key: "roll", label: "Roll", suffix: "°", min: -180, max: 180, step: 1 },
    ];

    return (
        <div className="fixed inset-0 z-[2000] flex flex-col bg-[#0d0f12] text-white" data-canvas-no-zoom onClick={onClose}>
            <header className="relative z-10 flex h-16 shrink-0 items-center justify-between border-b border-white/10 px-5" onClick={(event) => event.stopPropagation()}>
                <div className="flex items-center gap-3">
                    <div className="grid size-9 place-items-center rounded-lg bg-white/10 text-white"><Rotate3d className="size-4" /></div>
            ...
            {hasPrev || hasNext ? <>
                <button type="button" disabled={!hasPrev} title="上一张" aria-label="上一张" onClick={(event) => { event.stopPropagation(); onPrev?.(); }} className="absolute left-4 top-1/2 z-20 grid size-10 -translate-y-1/2 place-items-center rounded-full border border-white/10 bg-white/5 text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-20"><ChevronLeft className="size-5" /></button>
                <button type="button" disabled={!hasNext} title="下一张" aria-label="下一张" onClick={(event) => { event.stopPropagation(); onNext?.(); }} className="absolute right-4 top-1/2 z-20 grid size-10 -translate-y-1/2 place-items-center rounded-full border border-white/10 bg-white/5 text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-20"><ChevronRight className="size-5" /></button>
            </> : null}

            <div className="flex min-h-0 flex-1" onClick={(event) => event.stopPropagation()}>
...
async function hydrateCanvasImages(nodes: CanvasNodeData[]) {
    return Promise.all(
        nodes.map(async (node) => {
            ...
            const isGeneratedMedia = Boolean(node.metadata?.model || node.metadata?.imageTaskId || node.metadata?.imageTaskResultId || node.metadata?.videoTaskId || node.metadata?.videoTaskVideoId);
            const persistMedia = async (url: string, contentType: string) => {
                if (!isGeneratedMedia || !url || url.startsWith("/api/media/generated/")) return url;
                try {
                    return (await persistGeneratedMedia(url, contentType)).url || url;
                } catch {
                    return url;
                }
            };
            if ((node.type === CanvasNodeType.Video || node.type === CanvasNodeType.Audio) && node.metadata?.storageKey) return { ...node, metadata: { ...node.metadata, content: await resolveMediaUrl(node.metadata.storageKey, content) } };
            if (node.type === CanvasNodeType.Video && content) {
                const videoCandidateBatches = node.metadata?.videoCandidateBatches
                    ? await Promise.all(node.metadata.videoCandidateBatches.map(async (batch) => ({ ...batch, items: await Promise.all(batch.items.map(async (candidate) => ({ ...candidate, content: await persistMedia(candidate.content || "", candidate.mimeType || "video/mp4") }))) })))
                    : undefined;
                const persistedContent = await persistMedia(content, node.metadata?.mimeType || "video/mp4");
                return { ...node, metadata: { ...node.metadata, content: persistedContent, videoCandidateBatches } };
            }
                    ...
                    node.metadata.imageCandidateBatches.map(async (batch) => ({
                        ...
                        items: await Promise.all(
                            batch.items.map(async (candidate) => {
                                ...
                                if (!candidate.content?.startsWith("data:image/")) return { ...candidate, content: await persistMedia(candidate.content || "", candidate.mimeType || "image/png") };
                                if (isGeneratedMedia) return { ...candidate, content: await persistMedia(candidate.content, candidate.mimeType || "image/png") };
            ...
            if (!content.startsWith("data:image/")) return { ...node, metadata: { ...metadata, content: await persistMedia(content, metadata?.mimeType || "image/png") } };
            if (isGeneratedMedia) return { ...node, metadata: { ...metadata, content: await persistMedia(content, metadata?.mimeType || "image/png") } };
...
function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode, modelCosts?: Parameters<typeof modelCapabilityFor>[0]): AiConfig {
    ...
    const size = isPanoramaNodeType(node?.type) ? PANORAMA_IMAGE_SIZE : mode === "video" ? videoSizeForCapability(rawSize, capability, vquality) : mode === "image" ? imageSizeForCapability(rawSize, capability) : rawSize;
