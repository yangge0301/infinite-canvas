...
function VideoReferenceTile({ option, kind, label, theme, onAdd, onRemove, disabled = false }: { option?: CanvasVideoResourceOption; kind: CanvasVideoReferenceKind; label?: string; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onAdd?: () => void; onRemove?: () => void; disabled?: boolean }) {
    ...
    return <div className={`group relative size-12 shrink-0 overflow-visible rounded-md border ${disabled ? "cursor-not-allowed opacity-35 grayscale" : ""}`} style={{ borderColor: theme.node.stroke, background: theme.node.fill }} title={disabled ? `${option.label} 不适用于当前模式` : option.label} aria-disabled={disabled}>
