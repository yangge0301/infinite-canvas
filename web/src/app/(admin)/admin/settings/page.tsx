...
const emptyChannel: AdminModelChannel = { id: "", protocol: "openai", customProtocol: "general", name: "", baseUrl: "", apiKey: "", models: [], weight: 1, timeout: 600, enabled: true, remark: "" };
                                        ...
                                        <Table
                                            ...
                                            columns={[
                                                ...
                                                },
                                                {
                                                    ...
                                                    dataIndex: "type",
                                                    width: 120,
                                                    render: (_, item: AdminModelCost) => <Select className="!w-full" value={item.type} options={[{ value: "text", label: "文本" }, { value: "image", label: "图片" }, { value: "video", label: "视频" }, { value: "audio", label: "音频" }]} onChange={(type) => patchModelCost(form, setModelCosts, item, { ...defaultModelCapability(item.model, type as ModelType), displayName: item.displayName || item.model, credits: item.credits })} />,
                                                },
                                                {
                                                    title: "比例",
                                                    dataIndex: "ratios",
                                                    width: 260,
                                                    render: (_, item: AdminModelCost) => item.type === "image" || item.type === "video" ? <Select className="!w-full" mode="multiple" maxTagCount="responsive" value={item.ratios} options={(item.type === "image" ? imageRatioOptions : videoRatioOptions).map((value) => ({ value, label: value }))} onChange={(ratios) => patchModelCost(form, setModelCosts, item, { ratios })} /> : "",
                                                },
                                                {
                                                    title: "清晰度",
                                                    dataIndex: "resolutions",
                                                    width: 190,
                                                    render: (_, item: AdminModelCost) => item.type === "image" ? <Select className="!w-full" mode="multiple" maxTagCount="responsive" value={item.resolutions} options={imageResolutionOptions.map((value) => ({ value, label: value.toUpperCase() }))} onChange={(resolutions) => patchModelCost(form, setModelCosts, item, { resolutions })} /> : "",
                                                },
                                                {
                                                    title: "质量",
                                                    dataIndex: "qualities",
                                                    width: 230,
                                                    render: (_, item: AdminModelCost) => item.type === "image" ? <Select className="!w-full" mode="multiple" maxTagCount="responsive" value={item.qualities} options={[{ value: "high", label: "超高质量" }, { value: "medium", label: "高质量" }, { value: "low", label: "普通质量" }]} onChange={(qualities) => patchModelCost(form, setModelCosts, item, { qualities })} /> : "",
                                                },
                                                {
                                                    title: "最大个数",
                                                    dataIndex: "maxCount",
                                                ...
                                                },
                                                {
                                                    ...
                                                    width: 140,
                                                    render: (_, item: AdminModelCost) => item.type === "video" ? <Select className="!w-full" value={item.fixedDuration ? "true" : "false"} options={[{ value: "true", label: "是" }, { value: "false", label: "否" }]} onChange={(value) => patchModelCost(form, setModelCosts, item, { fixedDuration: value === "true" })} /> : "",
                                                },
                                                {
                                                    title: "固定时长选项",
                                                    dataIndex: "durationOptions",
                                                    width: 240,
                                                    render: (_, item: AdminModelCost) => item.type === "video" && item.fixedDuration ? <Select className="!w-full" mode="multiple" maxTagCount="responsive" value={item.durationOptions} options={videoDurationOptions.map((value) => ({ value, label: `${value}s` }))} onChange={(durationOptions) => patchModelCost(form, setModelCosts, item, { durationOptions })} /> : "",
                                                },
                                                {
                                                    title: "最长秒数",
                                                ...
                                                },
                                                {
                                                    ...
                                                    width: 260,
                                                    render: (_, item: AdminModelCost) => item.type === "video" ? <Select className="!w-full" mode="multiple" maxTagCount="responsive" value={item.videoQualities} options={videoQualityOptions.map((value) => ({ value, label: value.toUpperCase() }))} onChange={(videoQualities) => patchModelCost(form, setModelCosts, item, { videoQualities })} /> : "",
                                                },
                                                {
                                                    title: "音频",
                                                    dataIndex: "videoGenerateAudio",
                                                    width: 110,
                                                    render: (_, item: AdminModelCost) => item.type === "video" ? <Select className="!w-full" value={item.videoGenerateAudio ? "true" : "false"} options={[{ value: "true", label: "开启" }, { value: "false", label: "关闭" }]} onChange={(value) => patchModelCost(form, setModelCosts, item, { videoGenerateAudio: value === "true" })} /> : "",
                                                },
                                                {
                                                    title: "声音",
                                                    dataIndex: "audioVoices",
                                                    width: 260,
                                                    render: (_, item: AdminModelCost) => item.type === "audio" ? <Select className="!w-full" mode="multiple" maxTagCount="responsive" value={item.audioVoices} options={audioVoiceOptions.map((value) => ({ value, label: value[0].toUpperCase() + value.slice(1) }))} onChange={(audioVoices) => patchModelCost(form, setModelCosts, item, { audioVoices })} /> : "",
                                                },
                                                {
                                                    title: "格式",
                                                    dataIndex: "audioFormats",
                                                    width: 200,
                                                    render: (_, item: AdminModelCost) => item.type === "audio" ? <Select className="!w-full" mode="multiple" maxTagCount="responsive" value={item.audioFormats} options={audioFormatOptions.map((value) => ({ value, label: value.toUpperCase() }))} onChange={(audioFormats) => patchModelCost(form, setModelCosts, item, { audioFormats })} /> : "",
                                                },
                                                {
                                                    title: "语速",
                                                ...
                                                },
                                                {
                                                    ...
                                                    width: 130,
                                                    render: (_, item: AdminModelCost) => <Select className="!w-full" disabled={item.type !== "video"} value={item.type === "video" ? item.creditType : "request"} options={item.type === "video" ? [{ value: "request", label: "每次" }, { value: "second", label: "每秒" }] : [{ value: "request", label: "每次" }]} onChange={(creditType) => patchModelCost(form, setModelCosts, item, { creditType })} />,
                                                },
                                            ]}
                                        />
                                    </Col>
                ...
                >
                    <Form form={channelForm} layout="vertical" requiredMark={false} initialValues={emptyChannel}>
                        <Row gutter={16}>
                            ...
                            <Col span={12}>
                                <Form.Item name="customProtocol" label="私有定制协议" extra="仅视频模型使用，其他模型仍按通用流程请求">
                                    <Select
                                        options={[
                                            { label: "通用协议", value: "general" },
                                            { label: "kk私有协议", value: "kk" },
                                            { label: "mikito私有协议", value: "mikito" },
                                            { label: "mikito私有sora协议", value: "mikito_sora" },
                                        ]}
                                    />
                                </Form.Item>
                            </Col>
...
function normalizeChannel(item: Partial<AdminModelChannel> = {}): AdminModelChannel {
    return {
        ...
        customProtocol: item.customProtocol || "general",
        name: item.name || "",
